...
其中重要的方法：正则化，什么是正则化呢？正则化H就是加上条件的H，在这个加上条件的H下计算最小化的E<sub>in</sub>就是最小化E<sub>in</sub> + $\lambda$/N * **w**<sup>T</sup>**w**,其中参数$\lambda$越大，限制越强，表示希望**w**=0的越多，也就是H越小，更小的d<sub>EFF</sub>(有效的VC)
