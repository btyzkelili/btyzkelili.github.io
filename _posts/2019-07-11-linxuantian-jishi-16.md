...
可以得到结论：更大的λ，就是希望越少的w不为零，也就是限制更小的C
...
什么是正则化呢？正则化H就是加上条件的H，在这个加上条件的H下计算最小化的E<sub>in</sub>就是最小化E<sub>in</sub> + λ/N * **w**<sup>T</sup>**w**,其中参数$\lambda$越大，限制越强，表示希望**w**=0的越多，也就是H越小，更小的d<sub>EFF</sub>(有效的VC dimension)
...
对于参数λ：
![](/img/linxuant-jishi/14-15.PNG) 
选择λ依赖于noise，更多的noise需要更大的λ(相当于路况不好的时候要常踩刹车)，但我们并不知道有多少noise，那么该如何选择呢？
