...
* 思路：对于每个centre，要求centre左右第k位对应相同，centre有2n-1个，因为除了n个字符都可以做centre构成abcba型回文外，每两个字符间也可以作为centre，构成aabb型回文  
* 复杂度：O(n<sup>2</sup>)  O(1)
* 实现：  
```java
class Solution {
    public String longestPalindrome(String s){
        ...
        for(int i=0; i<=s.length()-1; i++){
            //以i为中心，abcba型的回文
            for(int k=1; i-k>=0 && i+k<=s.length()-1 && s.charAt(i-k)==s.charAt(i+k); k++)
                len1 = 1+2*k;
            //以i为中心，aabb型的回文
            for(int l=1; i-l+1>=0 && i+l<=s.length()-1 && s.charAt(i-l+1)==s.charAt(i+l); l++)
                len2 = 2*l;
...
**1.String**    
s.length()  
s1.concat(s2)，s1 + s2 +"..."：连接字符串
s.charAt(0)
s.equals(s2)  
s.substring(a,b):返回[a,b)下标的子串
